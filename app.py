...
def log_prediction(url, prediction, response_time, confidence):
    try:
        connection = get_db_connection()
        query = text("""
            INSERT INTO prediction_logs (url, prediction, response_time, confidence)
            VALUES (:url, :prediction, :response_time, :confidence)
        """)
...
def predict():
    try:
        ...
        confidence_score = model.predict_proba(url_features)[0][1]  # Probability of class 1 (phishing)
        response_time = time.time() - start_time

        # 📝 Log prediction
        log_prediction(url, result, response_time, confidence_score)
